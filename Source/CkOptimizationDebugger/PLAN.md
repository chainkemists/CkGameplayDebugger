...
| **P14** | Fix safety — the placement audit | **2026-08-21.** Three defects reported against the Fix button, all one omission: the conversion gate judged the SHAPE of a placement and never what it CARRIED. `Fixes/CkOptimizationDebugger_FixPreflight.{h,cpp}` — a named-reason audit replacing the yes/no gate (actor and component tags, editor Group, Data Layer, HLOD layer, dynamic material instance, inbound actor and Level-Blueprint references, plus a REFLECTION-driven template comparison so no property can be forgotten), two passes (absolute reasons against self, then comparison against the template), groups ranked by CONVERTIBLE count rather than candidate count, and a deterministic refusal summary that rides the success message as well as the failure one. Group removal before every `EditorDestroyActor`, mirroring `edactDeleteSelected`. A World-Position-Offset precondition refusing `Mesh.NaniteCandidate` on meshes whose materials animate vertices, with `Unknown` distinguished from `No`. A per-material blast-radius count on the shared-material fix | **Done** — gate green `Ck.OptimizationDebugger` **63/63, 2026-08-21**, against a captured baseline of 60/60 on the same binaries; the three new specs are `Preflight.{RefusalWording, RefusalSummary, ComparisonExclusions}`. The conversion and delete paths themselves stay `[EDITOR-VERIFY]` — no spec applies a fix |
| **P15** | Plan / apply split, preview, dirty list, fix log | Every fix gains a `Plan_*` that is READ-ONLY and returns per-property before → after, its side effects, and its refusal — which makes the planner the single home of the re-validation each apply used to carry. Preview dialog over the fix queue with per-property ticks; a modified-package list after apply routing saves through the editor's own checkout-and-save prompt; a session fix log with copy-as-commit-message | **Done** — gate green `Ck.OptimizationDebugger` **67/67, 2026-08-21** (63 → 67; the four new specs are `FixPlan.{IncludedWork, Summary, Drift, CommitMessage}`). NOTE: the first run of this gate reported 63/63 off the toolbox's CACHED test list and never scheduled the new specs — `--discover-fresh` is required after adding a spec without a `--build` in the same invocation |
| **P16** | Suppressions | One record (Finding / Asset / Folder / Check + reason + author + date), one pure matcher, project-config store that travels with the repo, plus the per-user tier `MutedStableKeys` migrates into. Suppressed findings render collapsed and counted, never hidden | **Done** — gate green **70/70, 2026-08-21** (+3: `Suppression.{ScopeMatching, ConfigRoundTrip, BuildForFinding}`) |
| **P17** | Project-wide scan | Registry-only first pass over `/Game` that opens nothing, then a non-modal cancelable deep pass. Findings for assets no open level places | **Done** — gate green **73/73, 2026-08-21** (+3: `ProjectScan.{RegistryChecks, UnknownIsNotZero, Progress}`). Registry pass answers 9 checks with nothing opened; the deep pass opens only what the registry cannot answer. Incremental on a repeating active timer, not an `FScopedSlowTask` — a modal cannot cancel in a second |
| **P18** | Audio family, findings export, heaviest-mesh ranking | **2026-08-21.** An AUDIO family living in the project scan’s deep pass, because `USoundWave` writes no cost-bearing registry tags: `Audio.MissingSoundClass` (a sound outside the project’s whole mix, which plays perfectly and is therefore never reported by anything else) and `Audio.LongSoundNotStreaming` against a new per-user threshold. `Model/CkOptimizationDebugger_FindingsReport.{h,cpp}` — the module’s long-standing known gap, a PURE HTML + Markdown builder over one sorted projection with the timestamp handed in, landing WITH the determinism spec the doctrine required. A project-wide heaviest-mesh ranking on the dashboard, ranked from the registry pass and EXCLUDING meshes whose triangle count the registry did not carry | **Done** — gate green `Ck.OptimizationDebugger` **76/76, 2026-08-21**. Two REAL failures were caught by this gate and fixed before it went green: the report sorted severity the wrong way round (the enum is declared worst-first, so "most severe first" is ASCENDING by value) and `k_CategoryCount` still said 7 after the Audio category landed |
| **P19** | Performance page — the PerfLab campaign's UI half | **2026-08-22.** Delivered by the [2026-08-21-PerfLab campaign](../../docs/campaigns/2026-08-21-PerfLab/PLAN.md), not by this plan. A seventh page, `Window/SCkPerfLabPage.{h,cpp}`, over the new `CkPerfLab` module: pick a map and a frame budget, press Run, and a separate `-game` process measures the level and writes a session this page reads back. Score with its eight disclosed components, evidence-gated findings, likely contributors, impact-vs-effort recommendations, session compare matched by position id, and HTML/CSV/JSON export. The viewport heatmap lives in the new `CkOptimizationDebuggerEditor` module — **this module still links no `UnrealEd`**, and decision #2 below is intact | **Done** — gate green `Ck.OptimizationDebugger` **76/76, 2026-08-22** (unchanged; the page adds UI, not test surface). `Ck.PerfLab` carries the logic and its own 60 specs. **`ECkOptimizationDebugger_Page` is append-only** — the switcher indexes by enum value, so `Performance` went last and `k_PageCount` went 6 → 7 |
