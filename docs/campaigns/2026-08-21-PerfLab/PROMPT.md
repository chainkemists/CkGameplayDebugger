...
| D2 | Child instance | `UnrealEditor-Cmd.exe <proj> <map> -game -windowed -unattended -nosplash` child process, file-based handoff (request JSON in, heartbeat + session JSON out under `Saved/CkPerfLab/Sessions/<id>/`) | Keeps main editor usable; `-game` minimizes editor overhead + `Saved/` contention |
...
| Second full **editor** child instance | Editor overhead pollutes numbers further; `Saved/` lock contention; slower boot |
