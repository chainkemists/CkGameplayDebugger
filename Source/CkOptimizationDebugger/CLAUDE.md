...
**Status: code-complete, partly verified, and the newest half is UNGATED.** Capture, storage, the viewer,
per-pixel identification and the selection/stats UI (P10/P11) are in and were gated green; the core loop has been
confirmed working in the editor by the maintainer. The QA round (P12: richer stats, resizable panel, HTML report,
`.cksnap` save/load) and the analysis round (P13: lenses, screen coverage, codec v2 with POV and capture context,
the mesh list, A/B compare, auxiliary views) were committed **without a build** at the maintainer's explicit
direction — no compile, no spec run. The owed gate and the outstanding `[EDITOR-VERIFY]` list are in
`docs/plans/2026-08-18-optimization-accuracy-and-snapshots/PROGRESS.md`.

One operational prerequisite: identification renders through the **`StencilId` CkUsf look**, whose master
material is generated by `Ck_Usf_GenerateLooks StencilId`. Until that master exists on disk, every capture
takes the degradation path — colour image, `HasIdMap == false`, and a notes line naming that exact cause —
and selection mode is disabled with the reason in its tooltip.
...
### Identity comes from stencil batches, and two fences make it work

Every pixel's mesh is recovered by rendering the candidates' CustomDepth-Stencil through CkUsf's `StencilId`
look — a post-process look declared as `.ush` plus an AngelScript asset, placed at `ReplacingTonemapper` so no
tone curve is applied to what are identifiers rather than colours. One pass distinguishes 255 primitives
(stencil 0 means "not in this pass"), so a capture runs `ceil(N / 255)` of them.

Two rules are what make it correct rather than plausible, and both are easy to break by "optimising":

- **Every candidate renders custom depth in EVERY pass.** Batch membership sets the stencil VALUE only. Gate
  the flag per batch instead and an out-of-batch occluder stops depth-testing the pass, so a hidden primitive
  stamps its id wherever the occluder is visible. The picture still looks reasonable; the ids are wrong.
- **All passes run in ONE game-thread scope.** Yield a frame and skeletal pose or world-position-offset time
  advances between passes, so the silhouettes disagree and pixels resolve as conflicts.

Foreign custom-depth users — CkUsf outlines and cel-shade subjects — are switched OFF for the capture, because
their stencil would be read back as whichever primitive shares its value. Every mutation goes through a ledger
whose restore guard is declared before the first write, so success, failure and early return all put the world
back, and the `r.CustomDepth` cvar is restored with them.

Rejected alternatives, so they are not re-proposed: editor hit proxies (game viewports have no hit-proxy
storage), ray-casting the live world at hover time (a stored snapshot outlives the world it pictured, which is
the point), and per-pixel bounds containment (bounds are far too loose at contact regions).

`Ck.OptimizationDebugger.Snapshot.{StencilBatching, PassResolution}` pin the slot maths and the resolve rule,
including the two answers that are not a prim: all-zero is sky or an excluded type, and a value past the end
of a partial last batch is a misread that resolves to nothing WITHOUT counting as a conflict.

### Proving it: the debug dump

Whether the ID map is a silhouette-exact copy of the picture is something a human sees instantly and no spec
can see at all, so the proof is two PNGs written side by side to `Saved/CkOptimizationDebugger/` behind the
`DumpSnapshotDebugImages` setting. The ID map is false-coloured per prim index with a SEEDED colour — adjacent
indices getting unrelated colours is what lets two meshes sharing an edge be told apart, which is exactly where
failures show. Soft edges mean the passes disagree about the point of view; a far mesh bleeding through a near
one means the render-every-pass rule broke; speckle means the identity target was not linear.
...
### One selector decides which image is on screen

`FCkOptimizationDebugger_SnapshotView` names it: the capture, a computed lens, or a stored auxiliary image. The
viewer caches exactly ONE brush for that view, rebuilt on change under a FRESH resource name (a renderer may cache
by name, so new bytes need a new name) and released on switch and on destruction. Eleven lenses at full capture size
would otherwise be eleven textures for a page the reader looks at one view of at a time.

A view the active snapshot cannot draw — a lens with no identification behind it, an auxiliary index it does not
carry — falls back to the capture rather than leaving an empty widget with the selector still claiming a view.

### Lenses are computed, auxiliary views are captured

Every per-mesh statistic is a per-pixel heatmap, because the ID map already says which mesh owns each pixel:
`Build_LensPixels` is pure over (snapshot, decoded ids, lens, thresholds) and paints triangle density, samplers,
texture and mesh memory, instances, distance, three masks, and a budget lens carrying `Get_Graded`'s own severity
colours. That is what makes these views work on a snapshot loaded from a `.cksnap` on a machine that never saw the
level, and in packaged Development where the engine's debug viewmodes are gone.

Three rules hold them honest:

- **Thresholds are handed in**, never read off the settings CDO — the checks' rule, for the checks' reasons.
- **Scalar ramps normalize over what is VISIBLE in this view**, and the legend says so. A mesh the capture recorded
  but nothing can see contributes no pixels and must not set the maximum, or the ramp flattens across everything on
  screen.
- **Every lens has a legend.** A heatmap whose units the reader guesses is worse than no measurement, and a red that
  means "worst here" is a different claim from the budget lens's red, which means "over a threshold".

Auxiliary views are the opposite trade: base colour and world normal are GBuffer reads a statistic cannot
reconstruct, captured through the same component at the same POV inside the same game-thread scope and stored as
PNGs. A failed auxiliary pass is not offered and the notes line says how many were dropped — it never costs the
picture. Depth and shader complexity are deliberately absent (PLAN.md decision 50).

### Screen coverage is the denominator

`Get_ScreenCoverage` counts pixels per prim off the same decode picking already pays for, and the viewer caches it
beside the decoded ids so the panel, the mesh list and every lens share one count. It is what turns "200k triangles"
into "200k triangles across forty pixels" — the honest over-detailed-for-how-it-is-seen metric, and the one number
no asset scan can produce. A snapshot without identification reports coverage as UNSET, never as zero: zero reads as
"not visible", which is a different claim.

### The mesh list and the picture are one selection

Rows are built on demand (collapsed means not built, because rows need coverage and coverage needs the ID map
decoded), sorted through one comparator per column with a prim-index tie-break that does NOT reverse with the header
arrow, and are row-safe throughout — text, boxes and the visual-only status pill, nothing that eats a left click.

Selection flows one way at a time: a click on the picture mutates the snapshot and then pushes into the list behind
`_SnapshotSelectionSyncGuard`; a list selection routes through `Apply_SnapshotSelection` and refreshes the overlay.
Without the guard each push fires the other surface's change callback and one click becomes an endless refresh.

### Comparing two captures

`Build_SnapshotDelta` is pure over two snapshots and keyed by mesh ASSET, not prim index — indices are per capture,
so an index-keyed diff reports every mesh as both added and removed. Placements of one asset aggregate into a single
row, mesh memory is the asset's (re-use is not new memory), coverage deltas are unset when either side lacks an ID
map, and the sort is total (triangle delta, then key) so two reads of one comparison cannot disagree about order.
