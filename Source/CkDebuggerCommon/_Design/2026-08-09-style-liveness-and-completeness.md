...
| `SurfaceElevation` | Layered / Flat / Outlined | Scoped to the common surface widgets only (Card, WindowChrome strips, InspectorPanel, LabeledGroup, ExpandableColumn) via `Get_SurfaceBrush(Depth)`/`Get_SurfaceTint(Depth)` — NOT a 230-site SBorder sweep. |
