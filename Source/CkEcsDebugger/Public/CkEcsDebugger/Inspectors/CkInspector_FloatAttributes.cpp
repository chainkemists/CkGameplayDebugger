...
namespace ck_inspector_float_attributes
{
    ...
    auto Make_Record(const FRecordProjection& InProjection) -> FCkUiRecordData
    {
        ...
        Record.Fields.Add(TEXT("current"), Make_NumberField(InProjection.Current));
        Record.Fields.Add(TEXT("min"), Make_NumberField(InProjection.Min));
        Record.Fields.Add(TEXT("max"), Make_NumberField(InProjection.Max));
        Record.Fields.Add(TEXT("modifier-delta"), Make_NumberField(InProjection.ModifierDelta));
    ...
    auto Is_AttributeLive(
        ...
        const FCk_Handle_FloatAttribute& InAttribute) -> bool
    {
        return NOT Is_Destroying(InOwner)
            && UCk_Utils_FloatAttribute_UE::Has_Any(InOwner)
            && NOT Is_Destroying(FCk_Handle{InAttribute})
            && UCk_Utils_FloatAttribute_UE::Has_Component(InAttribute, ECk_MinMaxCurrent::Current);
    }

    auto Get_Fraction(const FCk_Handle_FloatAttribute& InAttribute) -> float
    {
        if (ck::Is_NOT_Valid(InAttribute)
            || NOT UCk_Utils_FloatAttribute_UE::Has_Component(InAttribute, ECk_MinMaxCurrent::Min)
            || NOT UCk_Utils_FloatAttribute_UE::Has_Component(InAttribute, ECk_MinMaxCurrent::Max))
        { return 0.0f; }

        const auto MinValue = UCk_Utils_FloatAttribute_UE::Get_FinalValue(
            InAttribute, ECk_MinMaxCurrent::Min);
        const auto MaxValue = UCk_Utils_FloatAttribute_UE::Get_FinalValue(
            InAttribute, ECk_MinMaxCurrent::Max);
        const auto Current = UCk_Utils_FloatAttribute_UE::Get_FinalValue(
    ...
    auto Get_Annotations(const FCk_Handle_FloatAttribute& InAttribute) -> FString
    {
        if (ck::Is_NOT_Valid(InAttribute)) { return FString{}; }

        const auto Final = UCk_Utils_FloatAttribute_UE::Get_FinalValue(
            InAttribute, ECk_MinMaxCurrent::Current);
        const auto Base = UCk_Utils_FloatAttribute_UE::Get_BaseValue(
            InAttribute, ECk_MinMaxCurrent::Current);
        auto Result = FString{};
        if (Base != Final)
        ...
        const bool bInMeter) -> FString
    {
        ...
        const auto Current = UCk_Utils_FloatAttribute_UE::Get_FinalValue(
            InAttribute, ECk_MinMaxCurrent::Current);
        auto Result = ck::Format_UE(TEXT("{:.2f}"), Current);
        if (bInHasMax)
        {
            const auto MaxValue = UCk_Utils_FloatAttribute_UE::Get_FinalValue(
                InAttribute, ECk_MinMaxCurrent::Max);
            Result += bInMeter
                ? ck::Format_UE(TEXT(" / {:.2f}"), MaxValue)
                : ck::Format_UE(TEXT("  / {:.0f}"), MaxValue);
        }
...
auto SCkInspector_FloatAttributesAuthored::Construct(const FArguments& InArgs) -> void
{
...
SCkInspector_FloatAttributesAuthored::~SCkInspector_FloatAttributesAuthored()
{
    Release();
}

auto SCkInspector_FloatAttributesAuthored::Get_IsAvailable() const -> bool
{
    return _Active
        && NOT ck_inspector_float_attributes::Is_Destroying(_Entity)
        && UCk_Utils_FloatAttribute_UE::Has_Any(_Entity);
}

auto SCkInspector_FloatAttributesAuthored::Get_CanRequest() const -> bool
{
...
auto SCkInspector_FloatAttributesAuthored::Get_RequestDisabledReason() const -> FString
{
    if (NOT Get_IsAvailable()) { return TEXT("Float attributes are unavailable."); }
    return ck::DebugRequestGate::Evaluate(
        _Entity, ECk_DebugRequest_Requirement::LocalOk).Reason.ToString();
}

auto SCkInspector_FloatAttributesAuthored::Refresh_Records() -> bool
{
    if (NOT _Records.IsValid())
    {
        const FCkUiLoadResult CreateResult = FCkUiCollection::TryCreate({
            ...
            {TEXT("current"), ECkUiFieldKind::Number},
            {TEXT("min"), ECkUiFieldKind::Number},
            {TEXT("max"), ECkUiFieldKind::Number},
            {TEXT("modifier-delta"), ECkUiFieldKind::Number},
    ...
    auto Attributes = TMap<FString, FCk_Handle_FloatAttribute>{};
    auto Modifiers = TMap<FString, FCk_Handle_FloatAttributeModifier>{};
    auto Refills = TMap<FString, FCk_Handle_FloatAttributeRefill>{};
    ...
    if (Get_IsAvailable())
    {
        ...
        UCk_Utils_FloatAttribute_UE::ForEach(
            MutableOwner,
            [this, bShowEditControls, &Records, &Attributes, &Modifiers, &Refills,
             &RouteAttributeKeys, &RouteComponents](FCk_Handle_FloatAttribute InAttribute)
            {
                if (NOT ck_inspector_float_attributes::Is_AttributeLive(_Entity, InAttribute))
                { return; }
                ...
                const auto AttributeKey = ck_inspector_float_attributes::Get_StableKey(InAttribute);
                if (AttributeKey.IsEmpty() || Attributes.Contains(AttributeKey)) { return; }

                const auto bHasMin = UCk_Utils_FloatAttribute_UE::Has_Component(
                    InAttribute, ECk_MinMaxCurrent::Min);
                const auto bHasMax = UCk_Utils_FloatAttribute_UE::Has_Component(
                    InAttribute, ECk_MinMaxCurrent::Max);
                const auto bMeter = bHasMin && bHasMax;
                const auto Current = UCk_Utils_FloatAttribute_UE::Get_FinalValue(
                    InAttribute, ECk_MinMaxCurrent::Current);
                const auto Summary = ck_inspector_float_attributes::Get_Summary(
                ...
                auto Refill = FCk_Handle_FloatAttributeRefill{};
                if (UCk_Utils_FloatAttribute_UE::Has_RefillAttribute(InAttribute))
                ...
                const auto RefillState = bHasRefill
                    ? ck::Format_UE(TEXT("{} @ {:.2f}/s"),
                        ...
                        UCk_Utils_FloatAttributeRefill_UE::Get_FillRate(Refill))
                    : FString{};
                ...
                const auto bAttributeVisible = ck_inspector_float_attributes::Matches_Row(
                    _Filter, AttributeName, AttributeName, Summary);
                const auto bCurrentVisible = bComponentFilterVisible || bOverrideFilterVisible
                    || ck_inspector_float_attributes::Matches_Row(
                        _Filter, AttributeName, TEXT("Current override"), ck::Format_UE(TEXT("{:.2f}"), Current));
                const auto bMinVisible = bHasMin && (bComponentFilterVisible || bOverrideFilterVisible
                    || ck_inspector_float_attributes::Matches_Row(
                         _Filter, AttributeName, TEXT("Min override"), ck::Format_UE(TEXT("{:.2f}"),
                            UCk_Utils_FloatAttribute_UE::Get_FinalValue(InAttribute, ECk_MinMaxCurrent::Min))));
                const auto bMaxVisible = bHasMax && (bComponentFilterVisible || bOverrideFilterVisible
                    || ck_inspector_float_attributes::Matches_Row(
                         _Filter, AttributeName, TEXT("Max override"), ck::Format_UE(TEXT("{:.2f}"),
                            UCk_Utils_FloatAttribute_UE::Get_FinalValue(InAttribute, ECk_MinMaxCurrent::Max))));

                Records.Add(ck_inspector_float_attributes::Make_Record({
                    .Key = AttributeKey,
                    .Name = AttributeName,
                    .Summary = Summary,
                    .CurrentText = ck::Format_UE(TEXT("{:.2f}"), Current),
                    .MinText = bHasMin ? ck::Format_UE(TEXT("{:.2f}"),
                        UCk_Utils_FloatAttribute_UE::Get_FinalValue(
                            InAttribute, ECk_MinMaxCurrent::Min)) : FString{},
                    .MaxText = bHasMax ? ck::Format_UE(TEXT("{:.2f}"),
                        UCk_Utils_FloatAttribute_UE::Get_FinalValue(
                    ...
                    .Current = Current,
                    .Min = bHasMin ? UCk_Utils_FloatAttribute_UE::Get_FinalValue(
                        InAttribute, ECk_MinMaxCurrent::Min) : 0,
                    .Max = bHasMax ? UCk_Utils_FloatAttribute_UE::Get_FinalValue(
                        InAttribute, ECk_MinMaxCurrent::Max) : 0,
                    .Fraction = ck_inspector_float_attributes::Get_Fraction(InAttribute),
                ...
                {
                    if (NOT UCk_Utils_FloatAttribute_UE::Has_Component(InAttribute, Component))
                    { continue; }

                    auto ComponentModifiers = TArray<FCk_Handle_FloatAttributeModifier>{};
                    auto MutableAttribute = InAttribute;
                    UCk_Utils_FloatAttributeModifier_UE::ForEach(
                        MutableAttribute,
                        [&ComponentModifiers](FCk_Handle_FloatAttributeModifier InModifier)
                        {
                    ...
                    const auto ComponentName = ck_inspector_float_attributes::Get_ComponentName(Component);
                    for (const auto& Modifier : ComponentModifiers)
                    {
                        ...
                        const auto Delta = UCk_Utils_FloatAttributeModifier_UE::Get_Delta(
                            Modifier, Component);
                        const auto bModifierVisible = ck_inspector_float_attributes::Matches_Row(
                            _Filter, AttributeName, RowLabel, ck::Format_UE(TEXT("{:.2f} Remove"), Delta));
                        const auto ModifierKey = ck::Format_UE(
                            TEXT("modifier/{}/{}/{}"),
                            AttributeKey,
                            ComponentName,
                            ck_inspector_float_attributes::Get_StableKey(Modifier));

                        Records.Add(ck_inspector_float_attributes::Make_Record({
                            .Key = ModifierKey,
                            .Name = AttributeName,
                            .RowLabel = RowLabel,
                            .RowValue = ck::Format_UE(TEXT("{:.2f}"), Delta),
                    ...
                    const auto bClearVisible = ck_inspector_float_attributes::Matches_Row(
...
auto SCkInspector_FloatAttributesAuthored::Resolve_Attribute(
    const FString& InKey,
    FCk_Handle_FloatAttribute& OutAttribute) const -> bool
{
    OutAttribute = {};
    const auto* Candidate = _Attributes.Find(InKey);
    if (Candidate == nullptr
        || NOT ck_inspector_float_attributes::Is_AttributeLive(_Entity, *Candidate))
    { return false; }
    ...
    UCk_Utils_FloatAttribute_UE::ForEach(
        MutableOwner,
        [Candidate, &bFound](FCk_Handle_FloatAttribute InCurrent)
        {
...
auto SCkInspector_FloatAttributesAuthored::Resolve_Modifier(
    const FString& InKey,
    FCk_Handle_FloatAttributeModifier& OutModifier) const -> bool
{
    OutModifier = {};
    ...
    auto Attribute = FCk_Handle_FloatAttribute{};
    if (NOT Resolve_Attribute(*AttributeKey, Attribute)
        || NOT UCk_Utils_FloatAttribute_UE::Has_Component(Attribute, *Component)
        || ck_inspector_float_attributes::Is_Destroying(FCk_Handle{*Candidate}))
    { return false; }
    ...
    UCk_Utils_FloatAttributeModifier_UE::ForEach(
        MutableAttribute,
        [Candidate, &bFound](FCk_Handle_FloatAttributeModifier InCurrent)
        {
            if (InCurrent == *Candidate) { bFound = true; }
        },
        *Component);
...
auto SCkInspector_FloatAttributesAuthored::Resolve_Refill(
    const FString& InKey,
    FCk_Handle_FloatAttributeRefill& OutRefill) const -> bool
{
    OutRefill = {};
    ...
    if (NOT Resolve_Attribute(InKey, Attribute)
        || NOT UCk_Utils_FloatAttribute_UE::Has_RefillAttribute(Attribute))
    { return false; }
...
auto SCkInspector_FloatAttributesAuthored::Commit_Override(
    const FString& InKey,
    const float InValue,
    const ECk_MinMaxCurrent InComponent) -> void
{
    auto Attribute = FCk_Handle_FloatAttribute{};
    if (NOT FMath::IsFinite(InValue)
        || NOT Get_CanRequest()
        || NOT Resolve_Attribute(InKey, Attribute)
        || NOT UCk_Utils_FloatAttribute_UE::Has_Component(Attribute, InComponent))
    { return; }
...
auto SCkInspector_FloatAttributesAuthored::Commit_Modifier(
    const FString& InKey,
    const float InValue) -> void
{
    auto Modifier = FCk_Handle_FloatAttributeModifier{};
    if (NOT FMath::IsFinite(InValue)
        || NOT Get_CanRequest()
        || NOT Resolve_Modifier(InKey, Modifier))
    { return; }
    UCk_Utils_FloatAttributeModifier_UE::Override(Modifier, InValue);
}

auto SCkInspector_FloatAttributesAuthored::Remove_Modifier(const FString& InKey) -> void
{
    auto Modifier = FCk_Handle_FloatAttributeModifier{};
...
auto SCkInspector_FloatAttributesAuthored::Clear_Component(const FString& InKey) -> void
{
    const auto* AttributeKey = _RouteAttributeKeys.Find(InKey);
    const auto* Component = _RouteComponents.Find(InKey);
    ...
    UCk_Utils_FloatAttributeModifier_UE::ForEach(
        MutableAttribute,
        [&bHasVisibleModifier](FCk_Handle_FloatAttributeModifier InModifier)
        {
            if (ck::IsValid(InModifier)
                && NOT ck_inspector_attribute_rows::Get_IsInternalReplicationModifier(InModifier))
    ...
    UCk_Utils_FloatAttributeModifier_UE::Request_ClearAllModifiers(
        Attribute, *Component, {});
}

auto SCkInspector_FloatAttributesAuthored::Pause_Refill(const FString& InKey) -> void
{
    auto Refill = FCk_Handle_FloatAttributeRefill{};
    if (NOT Get_CanRequest() || NOT Resolve_Refill(InKey, Refill)
        || UCk_Utils_FloatAttributeRefill_UE::Get_RefillState(Refill)
            != ECk_Attribute_RefillState::Running)
    { return; }
    UCk_Utils_FloatAttributeRefill_UE::Request_Pause(Refill, {});
}

auto SCkInspector_FloatAttributesAuthored::Resume_Refill(const FString& InKey) -> void
{
    auto Refill = FCk_Handle_FloatAttributeRefill{};
    if (NOT Get_CanRequest() || NOT Resolve_Refill(InKey, Refill)
        || UCk_Utils_FloatAttributeRefill_UE::Get_RefillState(Refill)
    ...
    { return; }
    UCk_Utils_FloatAttributeRefill_UE::Request_Resume(Refill, {});
}

auto SCkInspector_FloatAttributesAuthored::Navigate(const FString& InKey) -> void
{
    auto Attribute = FCk_Handle_FloatAttribute{};
...
auto SCkInspector_FloatAttributesAuthored::Build_AuthoredView() -> bool
{
    ...
    const TWeakPtr<SCkInspector_FloatAttributesAuthored> WeakWidget{SharedThis(this)};
    auto Data = FCkUiView::FDataBindings{};
    Data.Visibility.Add(TEXT("float-attributes-available"),
        TAttribute<bool>::CreateLambda([WeakWidget]()
        {
            const auto Widget = WeakWidget.Pin();
            return Widget.IsValid() && Widget->Get_IsAvailable();
        }));
    Data.Visibility.Add(TEXT("float-attributes-can-request"),
        TAttribute<bool>::CreateLambda([WeakWidget]()
        {
            const auto Widget = WeakWidget.Pin();
            return Widget.IsValid() && Widget->Get_CanRequest();
        }));
    Data.Text.Add(TEXT("float-attributes-disabled-reason"),
    ...
    Data.Collections.Add(TEXT("float-attributes"), _Records);
    Data.ItemActions.Add(TEXT("float-attribute-navigate"),
    ...
    Data.ItemActions.Add(TEXT("float-attribute-remove"),
        FCkUiOnItemAction::CreateLambda([WeakWidget](const FString& InKey)
        {
            if (const auto Widget = WeakWidget.Pin(); Widget.IsValid())
            { Widget->Remove_Modifier(InKey); }
        }));
    Data.ItemActions.Add(TEXT("float-attribute-clear"),
        FCkUiOnItemAction::CreateLambda([WeakWidget](const FString& InKey)
        {
            if (const auto Widget = WeakWidget.Pin(); Widget.IsValid())
            { Widget->Clear_Component(InKey); }
        }));
    Data.ItemActions.Add(TEXT("float-attribute-refill-pause"),
        FCkUiOnItemAction::CreateLambda([WeakWidget](const FString& InKey)
        {
            if (const auto Widget = WeakWidget.Pin(); Widget.IsValid())
            { Widget->Pause_Refill(InKey); }
        }));
    Data.ItemActions.Add(TEXT("float-attribute-refill-resume"),
        FCkUiOnItemAction::CreateLambda([WeakWidget](const FString& InKey)
        {
            if (const auto Widget = WeakWidget.Pin(); Widget.IsValid())
            { Widget->Resume_Refill(InKey); }
        }));
    Data.ItemNumberCommitted.Add(TEXT("float-attribute-current-override"),
        FCkUiOnItemNumberCommitted::CreateLambda(
            [WeakWidget](const FString& InKey, const float InValue, ETextCommit::Type)
            {
                if (const auto Widget = WeakWidget.Pin(); Widget.IsValid())
                { Widget->Commit_Override(InKey, InValue, ECk_MinMaxCurrent::Current); }
            }));
    Data.ItemNumberCommitted.Add(TEXT("float-attribute-min-override"),
        FCkUiOnItemNumberCommitted::CreateLambda(
            [WeakWidget](const FString& InKey, const float InValue, ETextCommit::Type)
            {
                if (const auto Widget = WeakWidget.Pin(); Widget.IsValid())
    ...
    Data.ItemNumberCommitted.Add(TEXT("float-attribute-max-override"),
        FCkUiOnItemNumberCommitted::CreateLambda(
            [WeakWidget](const FString& InKey, const float InValue, ETextCommit::Type)
            {
                if (const auto Widget = WeakWidget.Pin(); Widget.IsValid())
    ...
    Data.ItemNumberCommitted.Add(TEXT("float-attribute-modifier"),
        FCkUiOnItemNumberCommitted::CreateLambda(
            [WeakWidget](const FString& InKey, const float InValue, ETextCommit::Type)
            {
                if (const auto Widget = WeakWidget.Pin(); Widget.IsValid())
    ...
    Candidate->SetFiles(
        FPaths::Combine(ResourceRoot, TEXT("EcsInspectorFloatAttributes.ui.html")),
        FPaths::Combine(ResourceRoot, TEXT("EcsInspectorFloatAttributes.ui.css")));
    Candidate->PollFiles();
...
auto SCkInspector_FloatAttributesAuthored::Tick(
...
auto SCkInspector_FloatAttributesAuthored::Release() -> void
{
...
FCkInspector_FloatAttributes::~FCkInspector_FloatAttributes()
{
    OnDeactivated();
}
...
{
    return NOT ck_inspector_float_attributes::Is_Destroying(Entity)
        && UCk_Utils_FloatAttribute_UE::Has_Any(Entity);
}

auto FCkInspector_FloatAttributes::Build_NativeBody(
    ...
    const FString& InFilter) -> TSharedRef<SWidget>
{
    ...
    if (ck_inspector_float_attributes::Is_Destroying(InEntity)
        || NOT UCk_Utils_FloatAttribute_UE::Has_Any(InEntity))
    { return Builder.Build(InEntity, InFilter); }

    auto MutableEntity = InEntity;
    UCk_Utils_FloatAttribute_UE::ForEach(
        MutableEntity,
        [&Builder, WeakSelectionModel](FCk_Handle_FloatAttribute InAttribute)
        {
            const auto AttributeTag = UCk_Utils_GameplayLabel_UE::Get_Label(InAttribute);
            const auto AttributeName = AttributeTag.IsValid()
                ? AttributeTag.ToString()
            ...
            if (bHasMin && bHasMax)
            {
                Builder.AddMeterRow(
                    ...
                    { return ck_inspector_float_attributes::Get_Fraction(InAttribute); }),
                    ECk_Tone::Accent,
                    TAttribute<FText>::CreateLambda([InAttribute]()
                    {
                        return FText::FromString(ck_inspector_float_attributes::Get_Summary(
            ...
            ck_inspector_attribute_rows::Add_OverrideRows<UCk_Utils_FloatAttribute_UE>(
                Builder, AttributeName, InAttribute);
            ck_inspector_attribute_rows::Add_RefillRows<
                UCk_Utils_FloatAttribute_UE,
                UCk_Utils_FloatAttributeRefill_UE>(Builder, AttributeName, InAttribute);
            ck_inspector_attribute_rows::Add_ModifierRows<
                UCk_Utils_FloatAttribute_UE,
                UCk_Utils_FloatAttributeModifier_UE>(Builder, AttributeName, InAttribute);
        });
    return Builder.Build(InEntity, InFilter);
}
...
auto FCkInspector_FloatAttributes::Build_Inspector(
    ...
    const FString& InFilter) -> TSharedRef<SWidget>
{
    ...
    if (CanInspect(Entity))
    {
        ...
        UCk_Utils_FloatAttribute_UE::ForEach(
            MutableEntity,
            [&CaptureDiff](FCk_Handle_FloatAttribute InAttribute)
            {
                const auto AttributeTag = UCk_Utils_GameplayLabel_UE::Get_Label(InAttribute);
                const auto AttributeName = AttributeTag.IsValid()
                    ? AttributeTag.ToString()
                ...
                for (const auto Component : {
                    ...
                    ECk_MinMaxCurrent::Max})
                {
                    ComponentCount += UCk_Utils_FloatAttribute_UE::Has_Component(
                    ...
                    ECk_MinMaxCurrent::Max})
                {
                    ...
                    UCk_Utils_FloatAttributeModifier_UE::ForEach(
                        ...
                        [&CaptureDiff, &AttributeName, &ComponentName](
                            FCk_Handle_FloatAttributeModifier InModifier)
                        {
    ...
    const TSharedRef<SCkInspector_FloatAttributesAuthored> Authored =
        SNew(SCkInspector_FloatAttributesAuthored)
            .Entity(Entity)
            .Filter(InFilter)
...
auto FCkInspector_FloatAttributes::Tick(const FCk_Handle&, float) -> void
{
    _Instances.RemoveAll([](const TWeakPtr<SCkInspector_FloatAttributesAuthored>& InInstance)
    {
...
auto FCkInspector_FloatAttributes::OnDeactivated() -> void
{
