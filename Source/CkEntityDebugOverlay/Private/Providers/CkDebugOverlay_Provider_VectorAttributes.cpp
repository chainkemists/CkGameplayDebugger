#include "CkDebugOverlay_Provider_VectorAttributes.h"
...
UE_DEFINE_GAMEPLAY_TAG(TAG_Ck_OnScreenDebugger_Provider_VectorAttributes,
    "Ck.OnScreenDebugger.Provider.VectorAttributes")
...
UE_DEFINE_GAMEPLAY_TAG(TAG_Ck_OnScreenDebugger_Provider_VectorAttributes_Value,
    "Ck.OnScreenDebugger.Provider.VectorAttributes.Value")

// --------------------------------------------------------------------------------------------------------------------

namespace
{
    FGameplayTag ProviderTag()    { return TAG_Ck_OnScreenDebugger_Provider_VectorAttributes; }
    FGameplayTag FieldTag_Value() { return TAG_Ck_OnScreenDebugger_Provider_VectorAttributes_Value; }
}

// --------------------------------------------------------------------------------------------------------------------

auto FCk_DebugOverlay_Provider_VectorAttributes::Get_ProviderTag() const -> FGameplayTag
{
    return ProviderTag();
}

auto FCk_DebugOverlay_Provider_VectorAttributes::Get_FieldTags() const -> TArray<FCk_DebugOverlay_FieldDesc>
{
    return {
        { FieldTag_Value(), true },
    };
...
auto FCk_DebugOverlay_Provider_VectorAttributes::CanProvide(const FCk_Handle& Entity) const -> bool
{
    if (ck::Is_NOT_Valid(Entity)) { return false; }
    return UCk_Utils_VectorAttribute_UE::Has_Any(Entity);
}

// --------------------------------------------------------------------------------------------------------------------

auto FCk_DebugOverlay_Provider_VectorAttributes::Collect(
    ...
    FCk_DebugOverlay_Section&              Out) -> void
{
    ...
    UCk_Utils_VectorAttribute_UE::ForEach(MutableEntity,
        [&Out, &Cfg](FCk_Handle_VectorAttribute InAttribute)
        {
            ...
            const auto FinalVal = UCk_Utils_VectorAttribute_UE::Get_FinalValue(
            ...
            Row.Value    = FText::FromString(AttributeName + TEXT(": ") +
                ck::Format_UE(TEXT("({:.1f}, {:.1f}, {:.1f})"), FinalVal.X, FinalVal.Y, FinalVal.Z));
...
auto FCk_DebugOverlay_Provider_VectorAttributes::Get_CompactToken(
    const FCk_Handle&                      Entity,
    const FCk_DebugOverlay_ProviderConfig& /*Cfg*/) const -> FString
{
    ...
    int32 Count = 0;
    UCk_Utils_VectorAttribute_UE::ForEach(MutableEntity,
        [&Count](FCk_Handle_VectorAttribute) { ++Count; });

    if (Count == 0) { return {}; }
    return FString::Printf(TEXT("V:%dattr"), Count);
}

// --------------------------------------------------------------------------------------------------------------------

CK_REGISTER_DEBUG_OVERLAY_PROVIDER(FCk_DebugOverlay_Provider_VectorAttributes)
