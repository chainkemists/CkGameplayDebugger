...
**Status: code-complete, visually unverified.** Capture, storage, the viewer, per-pixel identification and the
selection/stats UI are all in (P10/P11) and every automated gate is green — but nothing here has been
confirmed by a human looking at it, and this is a feature whose entire output is a picture. The plan and the
outstanding checks are in `docs/plans/2026-08-18-optimization-accuracy-and-snapshots/`.
