...
#include "CkEcsDebugger/Inspectors/CkInspector_IntegerAttributes.h"
...
namespace ck_inspector_integer_attributes_authored_test
{
    ...
    auto GetInteger(const FCkUiCollection& InRecords, const FString& InKey, const FString& InField) -> int32
    {
        const TSharedPtr<const FCkUiRecord> Record = InRecords.FindRecord(InKey);
        const FCkUiFieldValue* Value = Record.IsValid() ? Record->FindField(InField) : nullptr;
        return Value != nullptr && Value->Kind == ECkUiFieldKind::Integer ? Value->Integer : MIN_int32;
    }
    ...
    auto FindUnlabeledModifier(
        ...
        const ECk_MinMaxCurrent InComponent) -> FCk_Handle_IntegerAttributeModifier
    {
        auto Result = FCk_Handle_IntegerAttributeModifier{};
        UCk_Utils_IntegerAttributeModifier_UE::ForEach(
            InAttribute,
            [&Result](FCk_Handle_IntegerAttributeModifier InModifier)
            {
    ...
    struct FFixture final
    {
        FCk_Handle OwnerA;
        FCk_Handle OwnerB;
        FCk_Handle_IntegerAttribute Bounded;
        FCk_Handle_IntegerAttribute BoundedB;
        FCk_Handle_IntegerAttribute MaxOnly;
        FCk_Handle_IntegerAttributeRefill Refill;
        FCk_Handle_IntegerAttributeModifier CurrentModifier;
        FCk_Handle_IntegerAttributeModifier MinModifier;
        FCk_Handle_IntegerAttributeModifier MaxModifier;
    };
    ...
    auto CreateFixture(ck::FEcsWorld& InWorld, FFixture& Out) -> bool
    {
        ...
        Out.Bounded = UCk_Utils_IntegerAttribute_UE::Add(Out.OwnerA,
            MakeParams(TAG_Label_IntegerAttribute.GetTag(), 30, ECk_MinMax::MinMax, 10, 90, true),
            ECk_Replication::DoesNotReplicate);
        Out.MaxOnly = UCk_Utils_IntegerAttribute_UE::Add(Out.OwnerA,
            MakeParams(TAG_Label_ByteAttribute.GetTag(), 80, ECk_MinMax::Max, 0, 200),
            ECk_Replication::DoesNotReplicate);
        ...
        Out.MaxModifier = UCk_Utils_IntegerAttributeModifier_UE::Add_Revocable(Out.Bounded,
            TAG_Label_AnimPlan_State.GetTag(), ECk_AttributeModifier_Operation::Add, ParamsMax);
        Out.Refill = UCk_Utils_IntegerAttribute_UE::TryGet_RefillAttribute(Out.Bounded);
...
{
    using namespace ck_inspector_integer_attributes_authored_test;
    if (NOT FSlateApplication::IsInitialized()) { AddError(TEXT("Integer Attributes authored test requires Slate.")); return false; }

    UCkDebuggerStyleSettings* Style = UCkDebuggerStyleSettings::Get_Mutable();
    if (Style == nullptr) { AddError(TEXT("Integer Attributes authored test requires debugger style settings.")); return false; }
    ...
    if (NOT TestTrue(
        TEXT("public integer utility fixture creates bounded, max-only, and revocable modifiers"),
        CreateFixture(World, Fixture)))
    { return false; }

    auto Inspector = FCkInspector_IntegerAttributes{};
    ...
    if (NOT TestEqual(TEXT("A mounts authored Integer Attributes"), RenderedA->GetTypeAsString(), FString{TEXT("SCkInspector_IntegerAttributesAuthored")})
        || NOT TestEqual(TEXT("B mounts independent authored Integer Attributes"), RenderedB->GetTypeAsString(), FString{TEXT("SCkInspector_IntegerAttributesAuthored")}))
    { AddError(Inspector.Get_LastAuthoredLoadError()); return false; }

    const TSharedRef<SCkInspector_IntegerAttributesAuthored> AuthoredA = StaticCastSharedRef<SCkInspector_IntegerAttributesAuthored>(RenderedA);
    const TSharedRef<SCkInspector_IntegerAttributesAuthored> AuthoredB = StaticCastSharedRef<SCkInspector_IntegerAttributesAuthored>(RenderedB);
    ...
    const FString BoundedName = TAG_Label_IntegerAttribute.GetTag().ToString();
    const FString MaxOnlyName = TAG_Label_ByteAttribute.GetTag().ToString();
    ...
    { return false; }
    const float ExpectedFraction = static_cast<float>(
        static_cast<double>(UCk_Utils_IntegerAttribute_UE::Get_FinalValue(
            Fixture.Bounded, ECk_MinMaxCurrent::Current)
            - UCk_Utils_IntegerAttribute_UE::Get_FinalValue(Fixture.Bounded, ECk_MinMaxCurrent::Min))
        / static_cast<double>(UCk_Utils_IntegerAttribute_UE::Get_FinalValue(
    ...
    TestEqual(TEXT("max-only branch remains plain and projects signed integer Max"),
        GetInteger(*RecordsA, MaxOnlyKey, TEXT("max")), 200);
    ...
    TSharedPtr<SCkInspector_IntegerAttributesAuthored> DiffAuthored;
    {
        const FCkInspector_DiffMarkScope DiffScope{&Differing};
        DiffAuthored = StaticCastSharedRef<SCkInspector_IntegerAttributesAuthored>(
    ...
    const TSharedPtr<SCkUiRepeat> Repeat = ViewA->GetRepeat(TEXT("integer-attribute-records"));
    ...
    const TSharedPtr<SWidget> CurrentHost = FindTagged(
        BoundedItem.ToSharedRef(), TEXT("integer-attribute-current-input"));
    const TSharedPtr<SWidget> MinHost = FindTagged(
        BoundedItem.ToSharedRef(), TEXT("integer-attribute-min-input"));
    const TSharedPtr<SWidget> MaxHost = FindTagged(
        BoundedItem.ToSharedRef(), TEXT("integer-attribute-max-input"));
    ...
    const TSharedPtr<SButton> SelectionButton = FindButton(BoundedItem.ToSharedRef(), TEXT("integer-attribute-name"));
    const TSharedPtr<SButton> PauseButton = FindButton(
        BoundedItem.ToSharedRef(), TEXT("integer-attribute-refill-pause"));
    const TSharedPtr<SButton> ResumeButton = FindButton(
        BoundedItem.ToSharedRef(), TEXT("integer-attribute-refill-resume"));
    ...
    TestTrue(TEXT("physical Pause routes through the public refill request"),
        UCk_Utils_IntegerAttributeRefill_UE::Get_RefillState(Fixture.Refill)
    ...
    if (NOT TestTrue(TEXT("physical Current commit dispatches"),
        Commit(Slate, CurrentInput.ToSharedRef(), TEXT("-125"))))
    { return false; }
    const FCk_Handle_IntegerAttributeModifier CurrentOverride = FindUnlabeledModifier(
        Fixture.Bounded, ECk_MinMaxCurrent::Current);
    TestTrue(TEXT("negative Current commit is not byte-clamped"),
        ck::IsValid(CurrentOverride)
        && UCk_Utils_IntegerAttributeModifier_UE::Get_Delta(
            CurrentOverride, ECk_MinMaxCurrent::Current) == -125);
    if (NOT TestTrue(TEXT("physical Min commit dispatches"), Commit(Slate, MinInput.ToSharedRef(), TEXT("-255")))
        || NOT TestTrue(TEXT("physical Max commit dispatches"), Commit(Slate, MaxInput.ToSharedRef(), TEXT("512")))) { return false; }
    const FCk_Handle_IntegerAttributeModifier MinOverride = FindUnlabeledModifier(
        Fixture.Bounded, ECk_MinMaxCurrent::Min);
    const FCk_Handle_IntegerAttributeModifier MaxOverride = FindUnlabeledModifier(
        Fixture.Bounded, ECk_MinMaxCurrent::Max);
    TestTrue(TEXT("signed and above-byte-range commits retain integer values"),
        ck::IsValid(MinOverride) && ck::IsValid(MaxOverride)
        && UCk_Utils_IntegerAttributeModifier_UE::Get_Delta(
            MinOverride, ECk_MinMaxCurrent::Min) == -255
        && UCk_Utils_IntegerAttributeModifier_UE::Get_Delta(
            MaxOverride, ECk_MinMaxCurrent::Max) == 512);
    ...
    const TSharedPtr<SEditableTextBox> ModifierInput = BoundedItem.IsValid()
        ? FindEditor(BoundedItem.ToSharedRef(), TEXT("integer-attribute-modifier-input")) : nullptr;
    const TSharedPtr<SButton> RemoveButton = BoundedItem.IsValid()
        ? FindButton(BoundedItem.ToSharedRef(), TEXT("integer-attribute-remove")) : nullptr;
    ...
    { return false; }
    Commit(Slate, ModifierInput.ToSharedRef(), TEXT("1024"));
    TestEqual(TEXT("modifier delta commits synchronously without byte clamping"),
        UCk_Utils_IntegerAttributeModifier_UE::Get_Delta(Fixture.CurrentModifier), 1024);
    ...
    const TSharedPtr<SButton> ClearMinButton = ClearMinItem.IsValid()
        ? FindButton(ClearMinItem.ToSharedRef(), TEXT("integer-attribute-clear-action")) : nullptr;
    ...
    const TSharedPtr<SButton> HeldMaxRemove = MaxModifierItem.IsValid()
        ? FindButton(MaxModifierItem.ToSharedRef(), TEXT("integer-attribute-remove")) : nullptr;
    ...
    const TSharedPtr<SButton> HeldMaxClear = ClearMaxItem.IsValid()
        ? FindButton(ClearMaxItem.ToSharedRef(), TEXT("integer-attribute-clear-action")) : nullptr;
    ...
    if (NOT TestTrue(TEXT("installed Integer Attributes resources are readable"), Plugin.IsValid()
        && FFileHelper::LoadFileToString(Markup, *FPaths::Combine(ResourceRoot, TEXT("EcsInspectorIntegerAttributes.ui.html")))
        && FFileHelper::LoadFileToString(Stylesheet, *FPaths::Combine(ResourceRoot, TEXT("EcsInspectorIntegerAttributes.ui.css"))))) { return false; }
    TestTrue(TEXT("resource is complete HTML/CSS ownership with no native port"), Markup.Contains(TEXT("<int32-input"))
        && Markup.Contains(TEXT("integer-attribute-refill-pause"))
        && Markup.Contains(TEXT("integer-attribute-refill-resume"))
        && Markup.Contains(TEXT("integer-attribute-clear")) && NOT Markup.Contains(TEXT("<native"))
        && Stylesheet.Contains(TEXT(".integer-attribute-row")));
    TestTrue(TEXT("compatible reload is accepted"), ViewA->TryReload(Markup, Stylesheet, TEXT("IntegerAttributes compatible")).Succeeded);
    ...
    TestFalse(TEXT("missing keyed typed handler rejects atomically"), ViewA->TryReload(
        Markup.Replace(TEXT(" item-committed=\"integer-attribute-current-override\""), TEXT("")), Stylesheet,
        TEXT("IntegerAttributes missing current handler")).Succeeded);
    ...
    const TSharedRef<SCkInspector_IntegerAttributesAuthored> FilteredAuthored =
        StaticCastSharedRef<SCkInspector_IntegerAttributesAuthored>(Filtered);
    ...
    const TSharedRef<SCkInspector_IntegerAttributesAuthored> ComponentFiltered =
        StaticCastSharedRef<SCkInspector_IntegerAttributesAuthored>(
    ...
    const TSharedRef<SCkInspector_IntegerAttributesAuthored> OverrideFiltered =
        StaticCastSharedRef<SCkInspector_IntegerAttributesAuthored>(
    ...
    const TSharedRef<SCkInspector_IntegerAttributesAuthored> RefillFiltered =
        StaticCastSharedRef<SCkInspector_IntegerAttributesAuthored>(
    ...
    TestTrue(TEXT("refill relationship remains live before filtered projection"),
        ...
        && ck::IsValid(UCk_Utils_IntegerAttribute_UE::TryGet_RefillAttribute(Fixture.Bounded)));
    ...
    const TSharedRef<SCkInspector_IntegerAttributesAuthored> RefillStateFiltered =
        StaticCastSharedRef<SCkInspector_IntegerAttributesAuthored>(
    ...
    const TSharedRef<SCkInspector_IntegerAttributesAuthored> RefillControlFiltered =
        StaticCastSharedRef<SCkInspector_IntegerAttributesAuthored>(
    ...
    TestTrue(TEXT("held refill controls fail closed after Float backing-component loss"),
        ...
        && UCk_Utils_IntegerAttributeRefill_UE::Get_RefillState(Fixture.Refill)
            == ECk_Attribute_RefillState::Running);
    Fixture.Bounded.Try_Remove<ck::TFragment_RefillAttribute<FCk_Handle_IntegerAttributeRefill>>();
    PauseButton->SimulateClick();
